...
bundle exec rake watchbg > "$SRC_DIR/build.dev.log" 2>&1 &
bundle exec npm install > "$SRC_DIR/build.dev.log" 2>&1 &
...
tail -F ./target/akvo-flow/flow0.log "$SRC_DIR/build.dev.log" ./target/build.log
