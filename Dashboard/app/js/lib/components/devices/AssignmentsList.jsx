    ...
    <div>
      ...
      {assignments.length > 0 && (
        <table className="dataTable assignmentListTable">
          ...
          <thead>
            <tr>
              <th className={sortProperties.column == 'name' ? tableHeaderClass() : ''}>
                <a onClick={() => onSort('name')} onKeyDown={() => onSort('name')}>
                  {strings.name}
                </a>
              </th>
              <th className={sortProperties.column == 'startDate' ? tableHeaderClass() : ''}>
                <a onClick={() => onSort('startDate')} onKeyDown={() => onSort('startDate')}>
                  {strings.startDate}
                </a>
              </th>
              <th className={sortProperties.column == 'endDate' ? tableHeaderClass() : ''}>
