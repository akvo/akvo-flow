...
FLOW.FolderSurveySelectorView = FLOW.ReactComponentView.extend(observe({
    ...
    folderSurveySelector(parentId) {
        ...
        if (parentId != 0) {
            let parent = this.surveyGroups.find(sg => sg.keyId == parentId);
            for (var i=0; i<parent['ancestorIds'].length; i++) {
                let level = [{
                    keyId: 0,
                    parentId: null,
            ...
            let selectedSG = this.surveyGroups.find(sg => sg.keyId == parentId);
        ...
        this.reactRender(
            <div>
                {levels.map((sgs, index) => (
                    <select key={"level"+index} value={this.state.value} onChange={this.handleChange}>
                        {sgs.map(sg => (
