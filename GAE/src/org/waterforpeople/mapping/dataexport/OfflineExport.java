...
/**
 * This utility will produce a RawDateReport using data contained a set of
 * data.txt files from the phone.
 * 
 * The input is a directory housing 1 or more zip files obtained from the
 * FieldSurvey application. This utility will merge the data file from all of
 * the zips into a single file and will then iterate over the merged text,
 * writing the responses to an excel raw data report.
 * 
 * This utility needs connectivity to the server to run ONLY to get the list of
 * questions and their IDs. The data will NOT be loaded into the server.
 * 
 * This utility will only work with data files generated via version 1.7 or
 * later of the FieldSurvey application
 * 
