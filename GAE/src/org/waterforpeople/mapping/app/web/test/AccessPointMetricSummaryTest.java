...
public class AccessPointMetricSummaryTest {
	...
	public void runTest(HttpServletResponse resp) {
		...
		for (AccessPoint ap : apList) {
			...
			List<AccessPointMetricSummary> apmsList = apmsDao.listMetrics(prototype);
			if(apmsList==null||apmsList.isEmpty()){
				prototype.setCount(1L);
				apmsDao.save(prototype);
				System.out.println("Adding new prototype metric " + prototype.toString());
			}else{
				for(AccessPointMetricSummary apmsItem: apmsList){
					...
					System.out.println("Incrementing count of prototype" +apmsItem.toString()+ " from " + oldCount + " to " + newCount);
				}
			}
		}
