...
public class TestHarnessServlet extends HttpServlet {
	...
	SurveyDAO surveyDAO = new SurveyDAO();
	ProjectFolderDAO projectFolderDAO = new ProjectFolderDAO();

	if (!projectFolderDAO.list(Constants.ALL_RESULTS).isEmpty())
	    return;

	List<SurveyGroup> allSurveyGroups = surveyGroupDAO.list(Constants.ALL_RESULTS);

	for (SurveyGroup surveyGroup : allSurveyGroups) {
	    if (!surveyGroup.getMonitoringGroup()) {
		ProjectFolder projectFolder = new ProjectFolder(surveyGroup.getName(), null);
		...
		for (Survey survey : surveys) {
		    ...
		    sg.setName(survey.getName());
		    sg.setCode(survey.getName());
		    sg.setProjectFolderId(projectFolder.getKey().getId());
		    sg.setMonitoringGroup(false);
		    surveyGroupDAO.save(sg);
