...
public class SurveyEditWidget extends Composite implements ContextAware, ChangeHandler {
	...
	if(validateInput()){
		...
		currentDto.setDescription(descriptionBox.getText()!= null? descriptionBox.getText().trim():null);
		final MessageDialog savingDialog = new MessageDialog(
				"Saving...",
				"Please wait.",true);
		savingDialog.showRelativeTo(panel);
		surveyService.saveSurvey(currentDto, groupDto.getKeyId(), new AsyncCallback<SurveyDto>() {
		...
		MessageDialog validationDialog = new MessageDialog(
				...
				"The survey code must contain at least 1 character.");
	...
	public void setContextBundle(Map<String, Object> bundle) {
		...
		groupDto = (SurveyGroupDto)bundle.get(BundleConstants.SURVEY_GROUP_KEY);
		populateWidgets();
	}
	...
	public void onChange(ChangeEvent event){
		if(currentDto != null){
			if(currentDto.getCode() != null && !currentDto.getCode().equals(codeBox.getText())){
				isChanged = true;
			}else if (currentDto.getDescription() != null && !currentDto.getDescription().equals(descriptionBox.getText())){
				isChanged = true;
			}else if ((ViewUtil.isTextPopulated(codeBox) && currentDto.getCode() == null) ||(ViewUtil.isTextPopulated(descriptionBox) && currentDto.getDescription() == null) ){
				isChanged = true;
			}else{
				isChanged = false;
			}
		}else{
			//if we haven't saved, set isChanged to true if either field is non empty
